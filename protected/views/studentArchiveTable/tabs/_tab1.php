	...
	<div class="row-left">
		...
		<?php
			echo $form->dropDownList($model,'student_transaction_branch_id',
				...
				array(
				'prompt' => '-----------Select-----------','tabindex'=>18,
				'ajax' => array(
				'type'=>'POST', 
				'url'=>CController::createUrl('StudentTransaction/getdivision'),	 	
	...
	<div class="row-right">
		...
		<?php 
			...
			if(isset($model->student_transaction_division_id))
				echo $form->dropDownList($model,'student_transaction_division_id', CHtml::listData(Division::model()->findAll(array('condition'=>'branch_id='.$model->student_transaction_branch_id.' and division_organization_id='.$org_id)), 'division_id', 'division_code'));
			else
			echo $form->dropDownList($model,'student_transaction_division_id' ,array(),
			array(
			'prompt' => '-----------Select-----------','tabindex'=>19,
			'ajax' => array(
			'type'=>'POST', 
			'url'=>CController::createUrl('StudentTransaction/getbatch'), 
