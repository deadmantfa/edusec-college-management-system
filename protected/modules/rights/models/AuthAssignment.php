...
class AuthAssignment extends CActiveRecord
{
	...
	public function GetRole($id)
	{
		$user = Yii::app()->db->createCommand()
		    ...
		    ->join('AuthItem i', 'a.itemname=i.name')
		    ->where('a.userid=:id AND i.type=:role', array(':id'=>$id,':role'=>2))
		    ->queryAll();
		$role_value = '';
		//print_r($user);
		$flag = false;
	...
	public function GetOperation($id)
	{
		$user = Yii::app()->db->createCommand()
		    ...
		    ->from('AuthAssignment a')
		    ->join('AuthItem i', 'a.itemname=i.name')
		    ->where('a.userid=:id AND i.type=:role', array(':id'=>$id,':role'=>0))
		    ->queryAll();
		$op_value = '';
		//print_r($user);
		$flag = false;
